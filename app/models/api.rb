...
class Api
  ...
  def send_payment(amount, recipient_id, token)
    request_headers = recipient_header(token)
    request_body = payment_body(amount, recipient_id)
    response = RestClient.post 'https://coolpay.herokuapp.com/api/payments', request_body, request_headers
  rescue RestClient::ExceptionWithResponse => exception
    response = exception.response
    response
  end
  ...
  def payment_body(amount, recipient_id)
    {
      payment: {
        amount: amount,
        currency: 'GBP',
